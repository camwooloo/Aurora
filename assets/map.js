...
(function () {
  ...
  function setLayer(name) {
    ...
    switch (name) {
      ...
      case 'satellite':
        ...
        break;
      case 'clouds':
        // Prefer RainViewer IR frames (10-min steps, animates smoothly),
        // fall back to MODIS TrueColor tiles when RainViewer's satellite
        // feed returns an empty array (which it sometimes does).
        if (state.satFrames.length) {
          setCloudFrame(state.satFrames.length - 1);
        } else {
          state.overlayLayer = buildGibsLayer(
            'MODIS_Terra_CorrectedReflectance_TrueColor',
            9, 'jpg', { opacity: 0.5, blend: 'screen' }
          ).addTo(state.map);
        }
        break;
      case 'snow':
  ...
  function setTime(hourOffset, totalHours) {
    ...
    if (state.currentLayer === 'clouds' && state.satFrames.length) {
      setCloudFrame(Math.floor(frac * state.satFrames.length));
    }
